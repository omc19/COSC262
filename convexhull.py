...
def giftwrap(listPts):
    ...
    k = h.index(Pk) #index
    chull = [] 
    while k != (len(h) - 1): #loop until index n-1
        h[i], h[k] = h[k], h[i] # Switch points
        min_Angle = 361
        chull.append(h[i])
...
def simpleClosedPath(listPts, min_y):
    ...
    angles.sort()
    for j in angles: #Append pts in order
...
def main():
    listPts = readDataPts('Set_A.dat', 10)  #File name, numPts given as example only
